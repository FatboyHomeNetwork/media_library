...
## TODO this loop is a bit shit. rework to remove duplicate code & optimise calls to __as_token()
        for c in value:
            if c.isalpha() or c.isnumeric() or c == "\'": 
                ...
                if len(s) > 0:
                    elements.extend(tokeniser.__as_token(s))
                    s = ''
...
################################################################################################
#
# Parser . This is where we apply the very simple rules for how the language works tot he tokens
