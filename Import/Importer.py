...
import file_based_queue as media_queue 
import system_config as config
import media_processing as mp
import media_library as ml

c = config.system_config(__file__) 

def prep_path(path):
    
    ml.open_permissions(path)
    ml.ensure_is_directory(path)
    ml.kill(path)
    
    path = ml.strip_unicode(path)
    path = ml.ensure_path_len(path)
